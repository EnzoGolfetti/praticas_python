...
#lista_itens = ['refresco', 'tempero', 'biscoito', 'salsicha', 'salame', 'paio', 'nuggets', 'steaks', 'hambúrguer', 'hamburguer', 'barbecue',
#               'molho de alho', 'molho de pimenta', 'molho inglês', 'ketchup', 'molho para salada', 'maionese', 'mostarda', 'shoyu']
...
for item in lista_itens:
    ...
    for id in ids:
        ...
        esperar(5)
        #Pass to an object
        lista_header = html_soup_product.find_all(string='Banco Brasileiro de Rótulos de Alimentos') #Participante não possui atividades
        print(lista_header)
        esperar(10)
        #Testa se houve preenchimento da lista com o header 
        if len(lista_header) > 0:
            text_sem_atividade = lista_header[0]
        else:
            text_sem_atividade = ''

        #Verifica se no Header tem o texto de erro
        if 'Banco Brasileiro de Rótulos de Alimentos' and 'Participante não possui atividades' not in text_sem_atividade:
            driver.refresh()
            esperar(10)
        ...
        elif 'Participante não possui atividades' in text_sem_atividade:
        ...
        #Se não tem o texto de erro vai para o forms do banco de dados
            ...
            try:
                ...
                #Aguarda
                esperar(5)
        # RETIRAR DEPOIS DE FINALIZAR TODOS OS TESTES ####################################
        #counter += 1
        #if counter == 6:
        #    break
...
#Loop de iteração em cada item coletado para extração das respostas das perguntas
for product in dict_forms_products:
    ...
    forms = dict_forms_products[product]
    ...
    dict_forms_products_2[product] = dict_respostas
    ...
    if (counter_product % 20) == 0:
        ...
        #Exporta o DataFrame
        df_temp.to_csv(f"banco_dados_tcc_julia_produtos_numero_{counter_product - 20}_a_{counter_product}_{str(datetime.datetime.today().date()).replace('-', '')}.csv",
